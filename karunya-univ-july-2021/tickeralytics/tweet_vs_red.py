...
def filter_data(data, hours, limit_rows=DEFAULT_LIMIT_ROWS):
    ...
    print(type(filtered_value_counts),"###########")
    ...
    print("AFTER RESET #######")
    print(filtered_value_counts_reset)
    filtered_value_counts_reset.to_csv("filtered_value.csv")
...
def twitter_analyse(lookback_hours):
        ...
        for tweet in tweets:
            ...
            if len(tweet.cashtags) > 0:
                ...
                for cashtag in tweet.cashtags:
                    ...
                    sentiment_on_tweet.append((y, cashtag.upper(),tweet.tweet))
                    print("ANALYSIS**********")
                    print(type(analysis))
                    print(analysis)
    ...
    data.to_csv('data_file.csv', encoding='utf-8')
    backup_tweet_data = pd.DataFrame(sentiment_on_tweet)

    backup_tweet_data.columns = [DATE_TIME_FIELD, TICKER_FIELD,"TWEET"]
    backup_tweet_data.date = pd.to_datetime(data.date)
    backup_tweet_data.to_csv('backup_data_file.csv', encoding='utf-8')
    ...
    global filtered_value_counts_reset
    hours_filtered, filtered_value_counts_reset = filter_data(data, lookback_hours, limit_rows)
    print("SECOND PRINT #########")
    print(hours_filtered)
    print("#############")
    print(filtered_value_counts_reset)
    global top_50_twitter
    top_50_twitter=list(filtered_value_counts_reset['ticker'])
...
def reddit_analyse(lookback_hours):
    ...
    reddit_mention = (alt.Chart(count_reset)
                      .mark_bar().encode(y=alt.Y('ticker', sort='-x'), x=alt.X('mentions')))
    global top_50_reddit
    top_50_reddit=list(count_reset['ticker'])
    global top_10_both
    top_10_both= [i for i in top_50_twitter if i in top_50_reddit]
    print("$$$$$$$$$$$$$$$$$$$$",type(top_50_reddit))
    print(top_50_reddit)
    print(top_50_twitter)
    print(top_10_both)

    final_reddit=count_reset.loc[count_reset['ticker'].isin(top_10_both)]
    final_twitter=filtered_value_counts_reset.loc[filtered_value_counts_reset['ticker'].isin(top_10_both)]
    print("REDDIT")
    print(final_reddit)
    print(type(final_reddit))

    print("TWITTER")
    print(final_twitter)
    fig2 = go.Figure(data=[
        go.Bar(name='REDDIT', x=list(final_reddit.ticker), y=list(final_reddit.mentions),marker={'color': final_reddit.mentions,
                                               'colorscale': 'hsv'}),
        go.Bar(name='TWITTER', x=list(final_twitter.ticker), y=list(final_twitter.mentions),marker={'color': final_twitter.mentions,
                                               'colorscale': 'hsv'})
    ])
    fig2.update_layout(barmode='group')
    st.plotly_chart(fig2)

    from graph_test import spl
    print("spl-----------------")
    print(spl)
    fig123 = px.bar(spl, x='ticker', y='mentions',
                 hover_data=['Vader Analysis', 'mentions'], color='Vader Sentiment',
                 labels={'mentions': 'Number of Mentions', 'ticker': 'Top 10 Tickers from Twitter & Reddit'},
                 height=400)
    ...
    with open('statistics.txt','a') as f:
        for i in top_10_both:
            f.write(i+',')
        f.write('\n')
    top_tickers = count_reset.ticker.to_list()

    of1 = open('top_10', 'wb')
    pickle.dump(top_10_both, of1)
    of1.close()
